...
defmodule Mix.Tasks.Petal.New.WebTest do
  use ExUnit.Case
  import MixHelper
  ...
  test "new without args" do
    assert capture_io(fn -> Mix.Tasks.Petal.New.Web.run([]) end) =~
  ...
  test "new outside umbrella", config do
    in_tmp config.test, fn ->
      assert_raise Mix.Error, ~r"The web task can only be run within an umbrella's apps directory", fn ->
        Mix.Tasks.Petal.New.Web.run ["007invalid"]
      end
    end
  end
