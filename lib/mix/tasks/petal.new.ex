defmodule Mix.Tasks.Petal.New do
  @moduledoc """
  Creates a new Phoenix project.

  It expects the path of the project as an argument.

      mix petal.new PATH [--module MODULE] [--app APP]
  ...
  `petal.gen.html`, `petal.gen.json`, `petal.gen.live`, and `petal.gen.context`
  ...
  `petal.gen.html` will no longer work, as important HTML components
  will be missing.

  ## Installation

  `mix petal.new` by default prompts you to fetch and install your
  dependencies. You can enable this behaviour by passing the
  `--install` flag or disable it with the `--no-install` flag.

  ## Examples

      mix petal.new hello_world
  ...
  """
  use Mix.Task
  alias Petal.New.{Generator, Project, Single, Umbrella, Web, Ecto}
  ...
  def run(argv, generator, path) do
    elixir_version_check!()
    case parse_opts(argv) do
      {_opts, []} -> Mix.Tasks.Help.run(["petal.new"])
  ...
  defp prompt_to_install_deps(%Project{} = project, generator, path_key) do
    ...
    maybe_cd(path, fn ->
      ...
      if path_key == :web_path do
        ...
        The PubSub server is typically defined in a `mix petal.new.ecto` app.
